...
fn read_input(path: Option<&Path>, read_stdin: bool, url: Option<&str>) -> anyhow::Result<InputDocument> {
...
fn html_redirect_target(html: &str, current_url: &Url) -> Option<Url> {
    if html.len() > 4096 {
        return None;
    }

    let document = Html::parse_document(html);
    if !looks_like_redirect_page(&document) {
        return None;
    }

    refresh_redirect_target(&document)
        .or_else(|| link_redirect_target(&document))
        .and_then(|target| current_url.join(&target).ok())
}

fn looks_like_redirect_page(document: &Html) -> bool {
    ...
    title_is_redirect || text.contains("redirected") || text.contains("redirecting")
}

fn refresh_redirect_target(document: &Html) -> Option<String> {
    ...
    document.select(&selector).find_map(|node| {
        ...
        refresh_content_url(value.attr("content")?)
    })
}
...
fn link_redirect_target(document: &Html) -> Option<String> {
